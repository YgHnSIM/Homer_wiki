...
- **굵은 글씨(볼드) 및 괄호 서식 규정**: 마크다운 파서(CommonMark)의 구두점-조사 인접 파싱 오류(`**표제어(원어)**는` 형태가 리터럴 `**`로 노출되는 결함)를 방지하고 가독성을 확보하기 위해, 원어 병기나 부가 설명 괄호는 볼드 마커 바깥으로 분리하여 `**한국어 표제어**(원어/외국어)조사` 또는 `**한국어 표제어** (원어/외국어)` 형식으로 작성합니다.
