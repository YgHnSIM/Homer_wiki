...
graph TD
    %% 기원 및 고대 그리스어 층위
    PG["선희랍 기층어 / 의성어<br/>(Pre-Greek Substrate / Uncertain)"] --> GRK["고대 희랍어: ἔλεος (eleos, 연민·비애감)"]
    GRK --> VERB["동사 파생: ἐλεέω (eleeō) / ἐλεαίρω (eleairō)"]
    VERB --> LXX["칠십인역/신약 코이네: ἐλεημοσύνη (eleēmosynē, 자비·자선)"]

    %% 라틴어 수용 층위
    LXX --> LLAT["교회 라틴어: eleēmosyna (자선·구제)"]
    LLAT --> VLAT["민중 라틴어 축약형: *alemosyna"]
    LLAT --> MLAT["후기/중세 라틴어 관직형: eleēmosynārius"]

    %% 3대 전파 분기 경로
    %% 경로 1: 게르만 고층 구어 차용 (Alms)
    VLAT --> OE["고대 영어: ælmesse (c. 950)"]
    OE --> ME1["중세 영어: almes (12~14세기)"]
    ME1 --> MOD1["현대 영어: alms (구제금 / [ɑːmz])"]

    %% 경로 2: 앵글로-노르만 불어 경유 제도어 (Almoner)
    MLAT --> OFR["고대 프랑스어: almosnier / aumônier"]
    OFR --> ME2["중세 영어: aumener (c. 1300)"]
    ME2 --> MOD2["현대 영어: almoner / almonry (자선관 / 자선원)"]

    %% 경로 3: 르네상스 직접 학술 차용 (Eleemosynary)
    MLAT -.->|"17세기 인문주의 직수입 (1612)"| MOD3["현대 영어: eleemosynary (자선의, 구제의)"]

    %% 경로 4: 전례 그리스어 직수입 (Kyrie eleison)
    VERB -.->|"전례 호격 명령구 직수입 (c. 1380)"| MOD4["전례 외래어: Kyrie eleison (키리에 엘레이손)"]
```
...
- 『**일리아스**』 **24권 503–506행, 516행** (프리아모스의 탄원과 아킬레우스의 화해):
  ...
  - **[주석]**: 탄원자 프리아모스가 요구하는 부정과거 명령형 `ἐλέησον`이 시신 반환이라는 즉각적·능동적 구호 결단을 촉구한다면, 아킬레우스의 현재분사 `οἰκτείρων`은 노인의 참혹한 굴욕을 목격한 관찰자의 감정적 억제와 신체적 부축 행위를 지칭합니다 ([[concept-oiktos|Oiktos]]).
