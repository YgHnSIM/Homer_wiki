...
8. **콜아웃 및 인용 블록(`>`) 내 위키링크 금지**:
   - `> [!NOTE]`, `> [!WARNING]`, `> **요약**:` 등 콜아웃 및 인용 블록 내부에는 `[[위키링크]]`를 일체 사용하지 않고 순수 텍스트(예: 아킬레우스, Menis, Xenia)로만 서술합니다.
9. **원문 인용구 콜아웃 적용 금지**:
   - 호메로스 원문 구절 및 연구 문헌/관용구 인용구는 `>` 인용 블록 대신 목록 항목(`- "..."`, `- *"..."*`) 또는 일반 따옴표/기울임꼴 단락으로 서술합니다.
...
1. **인덱스 및 로그 갱신**:
   ...
   - words/index.md에는 ## 작성된 수용 경로 mermaid 카탈로그를 추가하거나 복원하지 않습니다. 현황 표와 알파벳 색인만 갱신합니다.
...
| **words/index.md에 작성된 수용 경로 mermaid 카탈로그 추가** | 목록 도식은 현황 표와 알파벳 색인과 중복되고 경로를 왜곡한다. 복원 금지. |
