...
## [2026-08-31] fix(meta) | overview.md 서브그래프 컨테이너 제거 및 헤더 노드 카드 전환으로 렌더링 겹침 완전 해소

- **작업 개요**: Mermaid 렌더링 엔진의 서브그래프 라벨-노드 충돌 및 상단 엣지 관통 버그를 원천 차단하기 위해, 불안정한 `subgraph` 컨테이너를 제거하고 명시적인 '헤더 노드 카드' 기반 2열 수직 흐름으로 전면 개편함.
- **주요 개선 내용**:
  - **헤더 노드 카드 전환**: `Iliad` 및 `Odyssey` 축의 서브그래프 타이틀을 독립 노드 카드(`일리아스: 분노에서 연민으로`, `오뒷세이아: 귀향과 정의 회복`)로 전환하여 텍스트 잘림 및 테두리 충돌 원천 제거.
  - **정확한 2열 좌우 배치 고정**: `Time --> Iliad`(좌측 컬럼)와 `Xenia --> Odyssey`(우측 컬럼)의 대칭 인과 엣지를 통해 좌: 일리아스, 우: 오뒷세이아의 배치 순서와 균형미를 영구 고정.
- **검증**: `npm run check` (wiki validation, concept-source matrix, tooling tests) 및 `npm run check:greek` 100% 통과.
