...
graph TD
    ...
    I3 -.->|"언어적 진실성 대비 (직언 vs 메티스)"| O4
    I5 -.->|"갈등 종결 대비 (비극 공유 vs 절멸)"| O3
```
...
```mermaid
graph TD
    subgraph Sub1["[1] 개인적 탁월성과 명예 체계"]
        A["탁월성 (Arete / 무력·지혜)"] -->|공동체 인정 요구| B["명예와 지위 (Timê / Geras)"]
        B -->|불멸의 시적 기억| C["불멸의 명예 (Kleos Aphthiton)"]
    end

    subgraph Sub2["[2] 사회적·종교적 억제 및 화해 메커니즘"]
        B -->|외적 비난 예견| D["정당한 도덕적 분노 (Nemesis)"]
        D -->|내면화된 양심| E["수치심과 자기억제 (Aidos)"]
        E -->|환대 규범| F["신성한 환대 (Xenia)"]
        E -->|탄원 수용| M["신성한 탄원 (Hikesia)"]
        M -->|필멸성 연대| N["비극적 연민 (Eleos)"]
    end

    subgraph Sub3["[3] 우주적 질서와 운명의 한계"]
        G["신성한 규칙 (Themis)"] -->|질서 위반 징벌| H["우주적 정의 (Dike / Tisis)"]
        I["필멸의 몫과 운명 (Moira / Aisa)"] -->|신적 눈멂| J["판단 마비 (Ate)"]
