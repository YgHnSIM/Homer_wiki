...
```mermaid
flowchart TD
    A["(1) 출전과 총사령관의 책무"] --> B["(2) 도성과 가족의 문턱"]
    B --> C["(3) 평원 돌파와 방벽 격파"]
    C --> D["(4) 함선 방화와 파트로클로스 사살"]
    D --> E["(5) 폴리다마스의 경고와 오판"]
    E --> F["(6) 성문 앞 고립과 결투"]
    F --> G["(7) 시신 훼손과 트로이아의 장례"]
```
...
헥토르의 전장 대결은 올림포스 신들의 운명 판정 아래 전개되며, 아카이아 3대 영웅과의 숙적 관계를 통해 파멸과 장례로 귀결된다(_Il._ 7.66–312, 16.818–861, 22.131–363, 24.477–676).
