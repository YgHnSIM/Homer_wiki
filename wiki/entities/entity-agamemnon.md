...
```mermaid
flowchart TB
    subgraph Axis1["[축 1] 아트레우스 가문 및 신성한 권력 계보 (Genealogy & Scepter)"]
        Zeus["제우스 (Zeus)"] ==>|스켑트론 하사| Her["헤르메스 (Hermes)"]
        Her --> Pel["펠롭스 (Pelops)"]
        Pel --> Atr["아트레우스 (Atreus)"]
        Atr --> Aga["아가멤논 (Agamemnon)<br/>미케네의 왕 / 총사령관"]
        Atr --> Men["메넬라오스 (Menelaos)<br/>스파르타의 왕"]
        Aga --- Cly["클뤼타임네스트라 (Clytemnestra)<br/>왕비"]
        Aga --- Chi["자녀들 (호메로스: 이피아나사·라오디케·크뤼소테미스·오레스테스)"]
    end

    subgraph Axis2["[축 2] 아카이아 연합군 지휘망 및 갈등 (Achaean Command & Friction)"]
        Aga -.->|티메/전리품 강탈 갈등| Ach["아킬레우스 (Achilles)<br/>최강의 전사"]
        Aga --- Ody["오디세우스 (Odysseus)<br/>정치적 조정 / 지략"]
        Aga --- Nes["네스토르 (Nestor)<br/>원로 자문"]
        Aga --- Dio["디오메데스 (Diomedes)<br/>전선 사기 진작 / 용맹"]
        Aga -.->|신탁 전달 갈등| Cal["칼카스 (Calchas)<br/>예언자"]
        Aga -.->|권력 남용 규탄/체벌| The["테르시테스 (Thersites)"]
    end

    subgraph Axis3["[축 3] 트로이아 원정 및 비극적 귀향 (Expedition & Tragic Nostos)"]
        Aga ==>|원정군 총지휘| Tro["트로이아 원정"]
        Aga -.->|사제 모욕 및 역병 유발| Chr["크뤼세스 (Chryses)"]
        Aga --- Cas["카산드라 (Cassandra)<br/>전리품 예언녀"]
        Aeg["아이기스토스 (Aegisthus)"] -.->|불륜 및 귀환 살해 공모| Cly
        Cly ==>|비극적 도살| Aga
        Aga -.->|저승에서의 경고/대비| OdyN["오디세우스의 귀향 (성공적 노스토스)"]
    end
```
...
### 10.1 서사시환(Epic Cycle)의 전승 변용

호메로스 서사시 본문에는 아울리스에서의 이피게네이아 희생 설화가 없다. 호메로스에서 아가멤논의 세 딸은 크뤼소테미스, 라오디케, 이피아나사로 온전히 생존해 있다(_Il._ 9.145). 이피게네이아의 인신공희와 사슴 대체 모티프는 서사시환 『퀴프리아』(_Cypria_ fr. 1)에서 처음 도입되어 후대 비극의 핵심 소재로 확립되었다.
...
| 호메로스 원전 내 이피게네이아 희생 부재 vs 비극 수용 | 서사시환 / 비극 | _Il._ 9.145 vs _Cypria_ fr. 1; Aesch. _Ag._ | 후대 수용 | [[concept-epic-cycle]] |
