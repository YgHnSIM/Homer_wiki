...
2. **정규 결투 중무장** (_Il._ 3.330–338): 메넬라오스와의 공식 결투에 임하면서 중장보병 갑주를 착용하지만, 시인은 **자신의 흉갑이 없어 형제 뤼카온의 흉갑을 입었으며 그것이 몸에 들어맞았다**(원문 `ἥρμο세`, 학술 전사 *hḗrmose*, _Il._ 3.333)라고 서술한다.
...
flowchart TB
    subgraph Divine["올림포스 신격 (Olympian Deities)"]
        Aph["아프로디테 (Aphrodite)<br/>선택 수증 · 3권 안개 구출 및 비호"]
        HerAth["헤라 & 아테나 (Hera & Athena)<br/>파리스의 심판으로 인한 트로이아 적대"]
        Apo["아폴론 (Apollo)<br/>트로이아 방어 · 화살 궤적 조력"]
    end
    ...
    subgraph Trojan["트로이아 왕실 (Trojan Royal House)"]
        ...
        Hel["헬레네 (Helene)<br/>약탈한 아내 · 경멸과 결합의 공존"]
        Oen["오이노네 (Oenone)<br/>첫 아내 님프 · 치료 거부"]
    end

    subgraph Achaean["아카이아 진영 (Achaean Rivals)"]
        Men["메넬라오스 (Menelaos)<br/>3권 1:1 결투 숙적 · 크세니아 피해자"]
        Dio["디오메데스 (Diomedes)<br/>11권 발등 관통 저격 피해자"]
        Ach["아킬레우스 (Achilles)<br/>스카이아이 성문 저격 표적"]
        Phi["필록테테스 (Philoktetes)<br/>헤라클레스의 활로 파리스 사살"]
    end

    Aph -->|"안개 구출 및 비호"| Par
    HerAth -.->|"심판으로 인한 적대"| Par
    Apo -->|"궁술 조력"| Par

    PriHek --> Par
    Hec <-->|"아이도스 질책 및 출전 독려"| Par
    Par <-->|"관능적 결합"| Hel
    Par -.->|"후대 전승: 독화살 치료 간청"| Oen
    Oen -.->|"치료 거부"| Par

    Par <-->|"결투 패배 및 대립"| Men
    Par -->|"11권 비석 은신처 저격"| Dio
    Par -.->|"후대 전승: 스카이아이 문 저격"| Ach
    Phi -.->|"후대 전승: 독화살 사살"| Par
```
...
- **학설 지형도**: 아서 애드킨스([[adkins-1960-merit-and-responsibility|Adkins 1960]])는 호메로스 사회가 근대적 의무·책임 개념이 부재하여 파리스처럼 '신들의 선물(`dôra theôn`)'이라는 핑계를 대는 '도덕적 결손'을 보인다고 분석한 반면, 버나드 윌리엄스([[williams-1993-shame-and-necessity|Williams 1993]])는 영웅들이 신적 개입 속에서도 자신의 의도와 행위에 따른 책임을 온전히 인식하고 있음을 논증하였다.
